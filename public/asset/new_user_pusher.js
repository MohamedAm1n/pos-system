...
channel.bind('App\\Events\\NewUser', function (data) {
    ...
    var newNotificationHtml = `<a href="${data.user_name}"><div class="media-body"><h6 class="media-heading text-right">${data.user_name}</h6> <p class="notification-text font-small-3 text-muted text-right">${data.comment}</p><small style="direction: ltr;"><p class="media-meta text-muted text-right" style="direction: ltr;">${data.date}${data.time}</p> </small></div></div></a>`;
